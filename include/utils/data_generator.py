...
class RealisticSalesDataGenerator:
    ...
    def generate_promotions(self) -> pd.DataFrame:
        ...
        major_events = [
            ...
            ('Christmas Sale', 12, 15, 10, 0.15), # Dec 15-24
            ('New Year Sale', 1, 1, 7, 0.20), # Jan 1-7
            ('Presidents Day', 2, 15, 3, 0.15), # Feb 15-17
            ('Memorial Day', 5, 25, 3, 0.15), # Last Monday of May
            ('July 4th Sale', 7, 1, 5, 0.15), # July 1-5
            ('Labor Day', 9, 1, 3, 0.15), # First Monday of September
            ('Back to School', 8, 1, 14, 0.10), # Aug 1-14
        ]
...
if __name__ == "__main__":
    ...
    sales_code_dir = os.path.dirname(os.path.dirname(os.path.dirname(script_path)))
    print("Script directory:", sales_code_dir)
    temp_dir = os.path.join(sales_code_dir, 'tmp/sales_data')

    generator = RealisticSalesDataGenerator(start_date="2023-11-01", end_date="2023-11-10")
    ...
    logging.info(f"Generating sales data from {generator.start_date} to {generator.end_date}")
    generator.generate_sales_data(output_dir=temp_dir)